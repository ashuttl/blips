...
gzipped JSON consumed at runtime by blips._basemap.  This is the single
source of truth for data/basemap.json.gz — it replaces the earlier per-layer
patch scripts (build_lakes.py, build_coast.py), which each re-implemented this
same simplify/area-filter logic to lift one layer at a time.  Run once at
authoring time against a directory of Natural Earth 1:10m/1:50m GeoJSON:

    NE_DIR=/path/to/geojson python3 prototype/build_basemap_data.py

The sister project linecast carries the same builder; the two are meant to
stay parallel copies (each project vendors its own), differing only in the
per-layer simplification density chosen for each map.

Output: src/blips/data/basemap.json.gz
"""
...
OUT = os.path.join(os.path.dirname(__file__), "..", "src", "blips",
                   "data", "basemap.json.gz")

# whole world (lon_min, lat_min, lon_max, lat_max)
REGION = (-180.0, -90.0, 180.0, 90.0)

# Simplification tolerance in degrees.  The drawn foreground the player reads
# against — coastlines and lake shorelines — is kept fine (~0.9 km); borders
# are dim political reference, imperceptible at approach zoom, so they stay at
# the coarser 1:50m density to save size.
EPS_COAST = 0.008
EPS_BORDER = 0.012
