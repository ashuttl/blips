...
(http://bixense.com/clicolors/), and BLIPS_COLOR for manual override.
"""
...
import os
import sys
from blips._theme import ensure_theme_loaded, theme_bg
...
def detect_color_mode(environ=None, stream=None):
    ...
    mode = _normalize_color_mode(env.get("BLIPS_COLOR", "auto"))
    if mode is None:
        mode = "auto"
    if mode != "auto":
        return mode
